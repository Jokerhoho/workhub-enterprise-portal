---
title: AI 工具库
description: WorkHub 企业 AI 工作台的工具教程入口，收录 WorkBuddy、千问办公与 Codex。
outline: false
---

# AI 工具库

先根据任务类型选工具，再进入对应教程。当前聚焦三个核心工具：WorkBuddy 负责跨文件与工作流，千问办公负责办公成果，Codex 负责代码仓库。

<div class="wb-tool-directory">
  <a class="wb-tool-directory__card wb-tool-directory__card--workbuddy" href="./workbuddy/">
    <span>通用工作 · WORKBUDDY</span>
    <h2>WorkBuddy</h2>
    <p>本地文件、知识处理、Skill、Agent、连接器与自动化。</p>
    <strong>进入教程 →</strong>
  </a>
...
| 你的任务 | 优先使用 | 交付重点 |
| --- | --- | --- |
| 整理本地文件、调用连接器、组合多步骤流程 | WorkBuddy | 可复用、可追踪、可暂停的任务流程 |
...
<style>
.wb-tool-directory { display: grid; grid-template-columns: repeat(3, minmax(0, 1fr)); gap: 16px; margin: 28px 0 36px; }
...
@media (min-width: 641px) and (max-width: 960px) { .wb-tool-directory { grid-template-columns: 1fr 1fr; } }
</style>
