...
const SITE_NAME = "WorkHub 企业 AI 工作台";
const SITE_ALTERNATE_NAME = "WorkHub";
const ORGANIZATION_NAME = "企业 AI 能力中心";
const DEFAULT_DESCRIPTION =
  "统一承载 WorkBuddy 知识、工具、案例与团队工作流的企业 AI 门户。";
...
function absolutePageUrl(siteUrl: string, page: string): string {
  return new URL(cleanPagePath(page).replace(/^\//, ""), `${siteUrl.replace(/\/$/, "")}/`).href;
}
  ...
  const itemListElement = [
    ...
    ...segments.map((segment, index) => {
      ...
      return {
        ...
        item: isLast
          ? canonicalUrl
          : new URL(parentPath.replace(/^\//, ""), `${siteUrl.replace(/\/$/, "")}/`).href,
      };
    }),
  ];
...
export function createSeoHead(
  ...
  const socialImageUrl = new URL("og/workhub-enterprise-portal.png", `${siteUrl.replace(/\/$/, "")}/`).href;
  ...
  const head: HeadConfig[] = [
    ...
    ["meta", { property: "og:image:alt", content: "WorkHub 企业 AI 工作台预览" }],
    ["meta", { name: "twitter:card", content: "summary_large_image" }],
    ["meta", { name: "twitter:title", content: title }],
    ["meta", { name: "twitter:description", content: description }],
    ["meta", { name: "twitter:image", content: socialImageUrl }],
    ["meta", { name: "twitter:image:alt", content: "WorkHub 企业 AI 工作台预览" }],
    [
      "script",
